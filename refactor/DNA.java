...
public class DNA {
		...
		while(noQuit){
			...
			if (s.contains("q")){
				...
				for (int i = 0; i < size; i++){
					code[i] = decision(s.toCharArray()[i]);
				}
	...
	public static BufferedReader opening(String fileName) throws Exception {
	...
	public static String inquire(int f) throws Exception {
		...
		if (!inp.contains("q")){
			if (inp.length() != f){
				...
				for (i = 0; i < f; i++){
					if ((inp.toCharArray()[i] != 'A')&&(inp.toCharArray()[i] != 'C')&&(inp.toCharArray()[i] != 'G')&&(inp.toCharArray()[i] != 'T')){
						System.out.println("You entered an invalid character: " + inp.toCharArray()[i] + " please re-enter string");
						throw new Exception();
					}
				}
			}
